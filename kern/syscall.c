...
sys_exofork(void)
{
	...
	struct Env *e = NULL;
	int status = env_alloc(&e, curenv->env_id);
	if (status == -E_NO_FREE_ENV || status == -E_NO_MEM) return status;

	if (!e || status < 0) panic("sys_exofork env_alloc=%d \n", status);
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (stat == -E_BAD_ENV) return stat;	
	if (!e || stat < 0) panic("sys_page_alloc envid2env=%d \n", stat);
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	stat = envid2env(srcenvid, &src_e, 1);
	if (stat == -E_BAD_ENV) return stat;	
	if (!src_e || stat < 0) panic("sys_page_map src envid2env=%d \n", stat);

	stat = envid2env(dstenvid, &dst_e, 1);
	if (stat == -E_BAD_ENV) return stat;	
	if (!dst_e || stat < 0) panic("sys_page_map dst envid2env=%d \n", stat);
...
{
	return e1000_try_recv_packet(page, out_len);
}

static int
sys_get_mac_low()
{
	return e1000_get_mac_low();
}

static int
sys_get_mac_high()
{
	return e1000_get_mac_high();
}
...
//	panic("syscall not implemented");
	...
	case SYS_try_recv_packet:
		return sys_try_recv_packet((char*) a1, (size_t*)a2);
	case SYS_get_mac_low:
		return sys_get_mac_low();
	case SYS_get_mac_high:
		return sys_get_mac_high();
	default: 
		return -E_INVAL;
	}
}
